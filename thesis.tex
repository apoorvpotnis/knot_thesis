...
\usepackage{microtype, lipsum, comment, physics2, tikz, tikz-3dplot, float, subcaption}
...
%\usepackage{embedall}
%\embedfile{apoorv_potnis_synopsis.bib}
...
%\addbibresource{apoorv_potnis_synopsis.bib}
...
\begin{document}
	...
	\begin{figure}[H]\centering
		...
		\qquad\quad\subcaptionbox{\(\tauu_i^{-1}\)}{
			...
			\end{tikzpicture}}
		\caption{Generators \(\tauu_i\) and \(\tauu_i^{-1}\). We have ommited the other straight strands.}
		...
		\subcaptionbox{\(\overline{gbg^{-1}}\)}{
			\begin{tikzpicture}
				\begin{knot}[clip width = 5]
					...
					\strand[thick, blue] (1,1) to [out=90,in=-90, blue] (0,2);
					\strand[thick, blue] (0,1) to [out=90,in=-90, blue] (1,2);
					\flipcrossings{1,2}
				\end{knot}
				\node at (-0.5,0.5) {\(g^{-1}\)};
				\node at (-0.5,1.5) {\(b\)};
...
related by a sequence of
\begin{enumerate}
	\item Rigid motions.
	\item Vertical and horizontal scalings, i.e.\ scalings along the \(z\)-axis and the \(x\)-axis.
	\item End-point fixing isotopies of the strands such that the isotopy is a \(n\)-strand set at all times.
\end{enumerate}

Translate \(b_2\) and \(b_1\) such that the first bottom point of \(b_1\) is the origin and the first bottom point of \(b_2\) agrees with the first top point of \(b_1\).

efine a group isomorphism by
\begin{align*}
	\phi &\colon \B_n \rightarrow \GB_n \\
	\phi &\colon \prod \sigma_i^{\pm 1} \mapsto \prod \tauu_i^{\pm 1}.
\end{align*}

Take a standard position geometric \(n\)-braid \(b\) and \(In\). Translate \(\In\) outside a bounded ball containing \(b\). Keeping the end points fixed, ambient isotope \(\In\) such that all the strands have a positive \(z\) coordinate, both the strand sets do not intersect, and the ambient isotopy must be an \(n\)-strand set at all times. Call the ambient isotoped set \(\In '\). Join the co-incident points of \(b\) and \(\In '\). The resulting set is called the closure of a geometric \(n\)-braid, denoted by \(\overline{b}\).
